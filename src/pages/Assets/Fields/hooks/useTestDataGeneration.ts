// src/pages/Assets/Fields/hooks/useTestDataGeneration.ts
...
export interface TestDataGenerationConfig {
  apiBaseUrl: string;
  onDataGenerated: (field: Partial<FieldData>) => void;
  onError: (error: any) => void;
  onStatusChange?: (status: string) => void;
}

/**
 * Хук для генерации тестовых данных полей
 */
export const useTestDataGeneration = (
  ...
  const generateTestData = useCallback(async () => {
    console.log('🎯 НАЧИНАЕМ generateTestData для поля через сервис');
    ...
    try {
      ...
      // Генерируем данные поля
      console.log('🎯 Генерируем данные поля через TestDataGeneratorService...');
      const generatedData = await testDataGeneratorRef.current!.generateFieldData();
      
      // Передаем сгенерированные данные через callback
      console.log('✅ Применяем сгенерированные данные поля...');
      config.onDataGenerated(generatedData);
      
      console.log('✅ Форма поля успешно заполнена тестовыми данными');
      
    } catch (err) {
      console.error('❌ Ошибка генерации тестовых данных поля:', err);
      config.onError(err);
    } finally {
