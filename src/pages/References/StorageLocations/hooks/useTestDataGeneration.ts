// src/pages/References/StorageLocations/hooks/useTestDataGeneration.ts
...
export interface TestDataGenerationConfig {
  apiBaseUrl: string;
  onDataGenerated: (storageLocation: Partial<StorageLocationData>) => void;
  onError: (error: any) => void;
  onStatusChange?: (status: string) => void;
}

/**
 * Хук для генерации тестовых данных мест хранения
 */
export const useTestDataGeneration = (
  ...
  const generateTestData = useCallback(async () => {
    console.log('🎯 НАЧИНАЕМ generateTestData для мест хранения через сервис');
    ...
    try {
      ...
      // Генерируем данные места хранения
      console.log('🎯 Генерируем данные места хранения через TestDataGeneratorService...');
      const generatedData = await testDataGeneratorRef.current!.generateStorageLocationData();
      
      // Передаем сгенерированные данные через callback
      console.log('✅ Применяем сгенерированные данные места хранения...');
      config.onDataGenerated(generatedData);
      
      console.log('✅ Форма места хранения успешно заполнена тестовыми данными');
      
    } catch (err) {
      console.error('❌ Ошибка генерации тестовых данных места хранения:', err);
      config.onError(err);
    } finally {
